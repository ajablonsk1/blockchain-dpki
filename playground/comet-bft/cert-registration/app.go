...
func NewPKIApplication() *PKIApplication {
	return &PKIApplication{&PKIState{make(map[string]*CertEntry)}}
}
...
func (app *PKIApplication) Query(_ context.Context, req *abcitypes.RequestQuery) (*abcitypes.ResponseQuery, error) {
	certEntry, exists := app.pkiState.Certs[string(req.Data)]
	if !exists {
	...
	if err != nil {
		return &abcitypes.ResponseQuery{Code: 2, Log: "error marshaling cert entry"}, nil
	}
	return &abcitypes.ResponseQuery{Code: 0, Value: certEntryJson}, nil
}
...
func (app *PKIApplication) FinalizeBlock(_ context.Context, req *abcitypes.RequestFinalizeBlock) (*abcitypes.ResponseFinalizeBlock, error) {
	...
	for i, tx := range req.Txs {
		...
		case "REGISTER":
			...
			app.pkiState.Certs[domain] = certEntry
		case "REVOKE":
